...
const GroupChatScreen = () => {
  ...
  const renderMessage = ({ item }) => {
    ...
    const senderName = isSystemMessage ? 'System Helper' : (profile.user?.first_name || item.sender?.first_name || 'Unknown');
    const isDownloaded = downloadedFiles.has(item.file_url);

    if (isSystemMessage) {
      return (
        <View style={tw`flex-row justify-center mb-2 px-4`}>
          <View style={tw`bg-gray-200 rounded-lg p-2 max-w-3/4`}>
            <Text style={tw`text-gray-800 text-sm text-center`}>{item.message}</Text>
      ...
      <View style={tw`flex-row mb-2 ${isCurrentUser ? 'justify-end' : 'justify-start'} px-4`}>
        <View style={tw`max-w-3/4 flex-row ${isCurrentUser ? 'flex-row-reverse' : ''}`}>
          {!isCurrentUser && (
            ...
            >
              <Image
                source={{ uri: profile.profile_picture || 'https://ui-avatars.com/api/?name=' + senderName + '&background=random' }}
